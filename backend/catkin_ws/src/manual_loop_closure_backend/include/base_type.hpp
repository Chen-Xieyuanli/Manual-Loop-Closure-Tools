...
/// A column vector of size 10.
using Vector10 = Eigen::Matrix<number_t, 10, 1>;
...
// 首先定义GNSS约束类型枚举
enum class GNSSConstraintType {
    GPS_XYZ,        // 原始的GPSFactor，只约束xyz位置
    PRIOR_POSE,     // PriorFactor<Pose3>，约束完整的Pose3
    XY_ONLY         // 自定义XY先验因子
};
...
extern bool align_gnss_enu_to_ros;
extern bool gnss_init_use_pose;
extern bool showDegenercy;
extern int degenercy_type;
...
extern bool useNoMotionFactor;

extern double gpsFrequence;
extern double gpsCovThreshold;
extern bool gpsRobustKernel;
extern bool loopRobustKernel;
extern bool gpsBatchOptimizationEnable;
extern int gpsBatchOptimizationThreshold;
extern double keyframeMeterGap;
extern double keyframeDegGap;
extern double keyframeRadGap;

extern double zuptTranslationThreshold;
extern double zuptRotationThreshold;
extern double pixelDis;
extern vector<double> origin_lla;

extern std::vector<double> runtime_origin_lla;
extern std::vector<double> runtime_origin_initial_lla;
extern int initialPoseType;
extern std::atomic<bool> runtime_origin_valid;
extern std::atomic<bool> runtime_origin_initial_valid;
extern std::mutex runtime_origin_mutex;
extern double pointCropRange;
extern Eigen::Vector3d pointCropBox;
extern bool pointCropBoxEnabled;
extern int mapSorMeanK;
extern double mapSorStdMul;
extern double mapRorRadius;
extern int mapRorMinNeighbors;
extern bool mapOutlierFilterEnabled;
extern std::string mapOutlierFilterMode;
extern bool useRefinedLidarCloud;
...
extern double loopMinOverlapThreshold;
extern double loopMaxCandidatePlanarDistance;

// Global map-to-frame (M2F) ICP tuning (map matching, not loop closure)
extern double m2fLocalMapRadius;        // local map crop radius (m)
extern double m2fIcpSearchRadius;      // ICP correspondence/search distance (m)
extern double m2fIcpScoreThreshold;    // max inlier RMSE to accept map ICP
extern double m2fIcpFitnessThreshold;  // min fitness/overlap to accept map ICP
extern bool   m2fUseHandcraftedICP;    // whether to use handcrafted LM-ICP instead of Open3D in M2F map matching

extern int historyKeyframeSearchNum;
extern int accumulatedFactorSize;
extern int gnss_constraint_type;
extern double loopClosureFrequency;
extern bool loopBackfillEnable;
extern double loopBackfillMoveDist;
extern double loopBackfillMoveYawDeg;
extern int loopBackfillPerCycle;
extern double loopBackfillRetryWaitSec;
extern int loopBackfillQueueMaxSize;
extern bool loopPCMEnable;
extern double loopPCMSelfTranslationThresh;
extern double loopPCMSelfRotationThreshDeg;
extern double loopPCMTranslationThresh;
extern double loopPCMRotationThreshDeg;
...
extern double grid_map_downsample_size;
extern double localization_map_grid_size;
extern double intensity_image_resolution;
extern bool save_grid_map;
extern bool save_intensity_image;

// Visualization and export controls
extern double pgoMapPubHz;           // /pgo_map 发布频率（Hz）
extern double pathPubHz;             // /pgo_path 发布频率（Hz）
extern double markerPubPeriod;       // MarkerArray 发布周期（秒）
extern bool exportCovariance;        // 是否在保存数据时导出并计算协方差
...
void LoadRosParams(ros::NodeHandle &nh, bool load_extrinsic = true);
