...
.PHONY: help venv check gui-help env-check backend assets clean
...
help:
	...
	@echo "  make backend    Build the offline optimizer backend"
...
check:
	$(PYTHON) -m py_compile launch_gui.py gui/manual_loop_closure_tool.py gui/manual_loop_closure/*.py gui/merge_pcds.py scripts/*.py
